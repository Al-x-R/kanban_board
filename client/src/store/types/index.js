export default {

  /**
   * AUTH ACTION TYPES
   */

  LOGIN: 'LOGIN',
  LOGIN_REQUEST: 'LOGIN_REQUEST',
  LOGIN_SUCCESS: 'LOGIN_SUCCESS',
  LOGIN_ERROR: 'LOGIN_ERROR',

  LOGOUT_REQUEST: 'LOGOUT_REQUEST',
  LOGOUT_SUCCESS: 'LOGOUT_SUCCESS',

  REGISTER_REQUEST: 'REGISTER_REQUEST',
  REGISTER_SUCCESS: 'REGISTER_SUCCESS',
  REGISTER_ERROR: 'REGISTER_ERROR',

  /**
   * NEW BOARD ACTION TYPES
   */
  CREATE_BOARD_REQUEST: 'CREATE_BOARD_REQUEST',
  CREATE_BOARD_SUCCESS: 'CREATE_BOARD_SUCCESS',
  CREATE_BOARD_ERROR: 'CREATE_BOARD_ERROR',
  RESET_NEW_BOARD_REDUCER: 'RESET_NEW_BOARD',

  /**
   * ALL BOARDS ACTION TYPES
   */
  GET_BOARDS_REQUEST: 'GET_BOARDS_REQUEST',
  GET_BOARDS_SUCCESS: 'GET_BOARDS_SUCCESS',
  GET_BOARDS_ERROR: 'GET_BOARDS_ERROR',

  /**
   * CURRENT BOARD ACTION TYPES
   */
  GET_BOARD_BY_ID_REQUEST: 'GET_BOARD_BY_ID_REQUEST',
  GET_BOARD_BY_ID_SUCCESS: 'GET_BOARD_BY_ID_SUCCESS',
  GET_BOARD_BY_ID_ERROR: 'GET_BOARD_BY_ID_ERROR',

  REMOVE_BOARD_BY_ID_REQUEST: 'REMOVE_BOARD_BY_ID_REQUEST',
  REMOVE_BOARD_BY_ID_SUCCESS: 'REMOVE_BOARD_BY_ID_SUCCESS',
  REMOVE_BOARD_BY_ID_ERROR: 'REMOVE_BOARD_BY_ID_ERROR',

  /**
   * COLUMNS BOARD ACTION TYPES
   */
  CREATE_COLUMN_REQUEST: 'CREATE_COLUMN_REQUEST',
  CREATE_COLUMN_SUCCESS: 'CREATE_COLUMN_SUCCESS',
  CREATE_COLUMN_ERROR: 'CREATE_COLUMN_ERROR',

  GET_COLUMNS_REQUEST: 'GET_COLUMNS_REQUEST',
  GET_COLUMNS_SUCCESS: 'GET_COLUMNS_SUCCESS',
  GET_COLUMNS_ERROR: 'GET_COLUMNS_ERROR',

  REMOVE_COLUMN_REQUEST: 'REMOVE_COLUMN_REQUEST',
  REMOVE_COLUMN_SUCCESS: 'REMOVE_COLUMN_SUCCESS',
  REMOVE_COLUMN_ERROR: 'REMOVE_COLUMN_ERROR',

  UPDATE_COLUMN_REQUEST: 'UPDATE_COLUMN_REQUEST',
  UPDATE_COLUMN_SUCCESS: 'UPDATE_COLUMN_SUCCESS',
  UPDATE_COLUMN_ERROR: 'UPDATE_COLUMN_ERROR',

  /**
   * CARDS BOARD ACTION TYPES
   */
  CREATE_CARD_REQUEST: 'CREATE_CARD_REQUEST',
  CREATE_CARD_SUCCESS: 'CREATE_CARD_SUCCESS',
  CREATE_CARD_ERROR: 'CREATE_CARD_ERROR',

  GET_CARDS_REQUEST: 'GET_CARDS_REQUEST',
  GET_CARDS_SUCCESS: 'GET_CARDS_SUCCESS',
  GET_CARDS_ERROR: 'GET_CARDS_ERROR',

  REMOVE_CARD_REQUEST: 'REMOVE_CARD_REQUEST',
  REMOVE_CARD_SUCCESS: 'REMOVE_CARD_SUCCESS',
  REMOVE_CARD_ERROR: 'REMOVE_CARD_ERROR',

  UPDATE_CARD_REQUEST: 'UPDATE_CARD_REQUEST',
  UPDATE_CARD_SUCCESS: 'UPDATE_CARD_SUCCESS',
  UPDATE_CARD_ERROR: 'UPDATE_CARD_ERROR',

  MOVE_CARD_IN_COLUMN: 'MOVE_CARD_IN_COLUMN',

  /**
   * ACTIVITIES BOARD ACTION TYPES
   */
  GET_BOARD_ACTIVITIES_REQUEST: 'GET_BOARD_ACTIVITIES_REQUEST',
  GET_BOARD_ACTIVITIES_SUCCESS: 'GET_BOARD_ACTIVITIES_SUCCESS',
  GET_BOARD_ACTIVITIES_ERROR: 'GET_BOARD_ACTIVITIES_ERROR',

  /**
   * ACTIVITIES CARD ACTION TYPES
   */
  GET_CARD_ACTIVITIES_REQUEST: 'GET_CARD_ACTIVITIES_REQUEST',
  GET_CARD_ACTIVITIES_SUCCESS: 'GET_CARD_ACTIVITIES_SUCCESS',
  GET_CARD_ACTIVITIES_ERROR: 'GET_CARD_ACTIVITIES_ERROR',

  /**
   * COMMENTS CARD ACTION TYPES
   */
  GET_COMMENTS_REQUEST: 'GET_COMMENTS_REQUEST',
  GET_COMMENTS_SUCCESS: 'GET_COMMENTS_SUCCESS',
  GET_COMMENTS_ERROR: 'GET_COMMENTS_ERROR',

  ADD_COMMENT_REQUEST: 'ADD_COMMENT_REQUEST',
  ADD_COMMENT_SUCCESS: 'ADD_COMMENT_SUCCESS',
  ADD_COMMENT_ERROR: 'ADD_COMMENT_ERROR',

  EDIT_COMMENT_REQUEST: 'EDIT_COMMENT_REQUEST',
  EDIT_COMMENT_SUCCESS: 'EDIT_COMMENT_SUCCESS',
  EDIT_COMMENT_ERROR: 'EDIT_COMMENT_ERROR',

  REMOVE_COMMENT_REQUEST: 'REMOVE_COMMENT_REQUEST',
  REMOVE_COMMENT_SUCCESS: 'REMOVE_COMMENT_SUCCESS',
  REMOVE_COMMENT_ERROR: 'REMOVE_COMMENT_ERROR',
};
